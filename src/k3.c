...
static bool my_connect(global_context_t* tcp_context, my_context handle, const struct sockaddr *serv_addr, socklen_t addrlen, int *err)
{
 ...
 //       struct sockaddr_in *temp; 
  ...
  //  printf("\naaafffffaaaaaaaaaaaaaaaaaa\n");
    ...
    tcp_header* header = 
        make_header((uint16_t)htons(cli->source->sin_port), sport, 0, 0, (uint8_t)0x50, (uint8_t)0x02, 3072, 0, 0);
...
static int my_app_dispatch_tcp(global_context_t* tcp_context, my_context handle, const void* data, size_t data_size)
{
    ...
    if(h->state==ESTAB)
    {
        tcp_data = malloc(20+data_size);
        printf("-------bbbbbbbbbbb\n");
        //printf("%d is sinport %d is destport\n",htons(h->source->sin_port),htons(h->dest->sin_port));
        header = make_header(htons(h->source->sin_port),htons(h->dest->sin_port), 1, 1, (uint8_t)0x50, (uint8_t)0x10, 3072, 0, 0);
        //printf("%d is sinport %d is destport\n",htons(h->source->sin_port),htons(h->dest->sin_port));

        printf("-------ccccccccccc\n");
        memcpy(tcp_data,header,20);
        memcpy(tcp_data+20,data,data_size);
        header->checksum = tcp_checksum(tcp_data, h->source->sin_addr, h->dest->sin_addr,  20+data_size,data_size);
        memcpy(tcp_data,header,20);
        printf("%x is source ip %x is dest ip\n",h->source->sin_addr,h->dest->sin_addr);
        written = tcp_context->ktcp_lib->tcp_dispatch_ip(h->source->sin_addr,h->dest->sin_addr,tcp_data,20+data_size);
        free(tcp_data);
        //h->seqnum += written;
        return written;
    }       
    ...
    if (handle->cstate == CONNECT){
        if (handle->state == SYN_SENT){
           ...
           //     printf("aaaaaaaaaaaaaaaaaaaaaa\n");
                ...
                list_remove(tcp_context->contextlist, handle);
                header = make_header(sport, dport, 1, htonl(header->seq_num)+1, (uint8_t)0x50, (uint8_t)0x10, 3072, 0, 0);
          ...
          //  printf("\n");
            ...
            //    printf("nnnnnnaaaaaaaaaaaaannnnnnnnnnnnn\n");
              ...
              //      printf("nnnnnnnnnnnnnnnnnnnnnnnnnnnn\n");
                    ...
                    tcp_header* header2 = make_header(htons(header->dest_port), htons(header->source_port), 0, htonl(header->seq_num)+1, (uint8_t)0x50, (uint8_t)0x12, 3072, 0, 0);
