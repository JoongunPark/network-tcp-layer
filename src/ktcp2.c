...
static void my_close(global_context_t* tcp_context, my_context handle,int *err)
{
    ...
    tcp_header* header = 
    make_header(htons(h->source->sin_port), htons(h->dest->sin_port), h->seqnum, h->acknum, (uint8_t)0x50, (uint8_t)0x01, 3072, 0, 0);
    h->seqnum++;
...
static bool my_connect(global_context_t* tcp_context, my_context handle, const struct sockaddr *serv_addr, socklen_t addrlen, int *err)
{
 ...
 //       struct sockaddr_in *temp; 
  ...
  //  printf("\naaafffffaaaaaaaaaaaaaaaaaa\n");
    ...
    tcp_header* header = 
        make_header((uint16_t)htons(cli->source->sin_port), sport, cli->seqnum, 0, (uint8_t)0x50, (uint8_t)0x02, 3072, 0, 0);
 ...
 //   printf("%d\n", list_get_count(h->pendinglist));
    ...
    //    printf("bbbbbbbbbbbbbbbbbbb\n");
       ...
       */ 
        ...
        new->source->sin_addr = t->sip;
        new->source->sin_port = t->header->dest_port;
    //    printf("%d %d\n",t->header->dest_port,htons(t->header->dest_port)); 
     ...
     //   printf("%d %d\n",t->header->source_port,htons(t->header->source_port)); 
        new->cstate = ACCEPT;
        new->state = SYN_RCVD;
        new->seqnum = h->seqnum+1;
        new->listenfd = h;
        new->start = 0;
        new->nc = 1;
        ...
        new->state = ESTAB;
        new->lastack = htonl(t->header->ack_num);
        new->acknum = htonl(t->header->seq_num);
        new->rwnd = htons(t->header->recieve_window);
...
static int my_app_dispatch_tcp(global_context_t* tcp_context, my_context handle, const void* data, size_t data_size)
{
    ...
    if(h->state==ESTAB || h->state == CLOSE_WAIT)
    {
            while(data_size > 0)
            {
                ...
                header = make_header(htons(h->source->sin_port),htons(h->dest->sin_port),h->seqnum, h->acknum, (uint8_t)0x50, (uint8_t)0x10, 3072, 0, 0);
    ...
    if (handle->state == CLOSE_WAIT){
      ...
      //  flush_buf(tcp_context, handle);
        ...
        while(handle->buflen > 0)
        {
            ...
            if(handle->rwnd - real_data_size < 0)
                break;
            void *tcp_data = malloc(20+real_data_size);
            header = make_header(htons(handle->source->sin_port),htons(handle->dest->sin_port),handle->seqnum, handle->acknum, (uint8_t)0x50, (uint8_t)0x10, 3072, 0, 0);
            memcpy(tcp_data,header,20);
        ...
        if (header->flags & 0x01){
            header = make_header(htons(handle->source->sin_port),htons(handle->dest->sin_port),handle->seqnum, handle->acknum+1, (uint8_t)0x50, (uint8_t)0x10, 3072, 0, 0);
    ...
    if (handle->cstate == CONNECT){
        if (handle->state == SYN_SENT){
           ...
           //     printf("aaaaaaaaaaaaaaaaaaaaaa\n");
                ...
                list_remove(tcp_context->contextlist, handle);
                handle->seqnum++;
                header = make_header(sport,dport,handle->seqnum, htonl(header->seq_num)+1, (uint8_t)0x50, (uint8_t)0x10, 3072, 0, 0);
            ...
            //    printf("nnnnnnaaaaaaaaaaaaannnnnnnnnnnnn\n");
              ...
              //      printf("nnnnnnnnnnnnnnnnnnnnnnnnnnnn\n");
                  ...
                  //  t->newstate = SYN_RCVD;
                    ...
                    tcp_header* header2 = make_header(htons(t->header->dest_port), htons(t->header->source_port), handle->seqnum, htonl(t->header->seq_num)+1, (uint8_t)0x50, (uint8_t)0x12, 3072, 0, 0);
                    header2->checksum = tcp_checksum(header2, t->sip, t->dip, 20, 0);
                    tcp_context->ktcp_lib->tcp_dispatch_ip(t->sip, t->dip, (void *)header2, 20);
...
*/
   ...
   //         printf("bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb\n");
            ...
            new->start = 0;
            tcp_header* header2 = make_header(htons(header->dest_port), htons(header->source_port), new->seqnum, htonl(header->seq_num)+1, (uint8_t)0x50, (uint8_t)0x12, 3072, 0, 0);
            new->seqnum++;
            new->nc = 1;
   //         printf("ccccccccccccccccccccccc\n");
         ...
         //
            ...
            handle->rwnd = htons(header->recieve_window);
            handle->buflen = 0;
      //      if (!tcp_context->ktcp_lib->tcp_passive_open(handle->listenfd, handle)) return;
        }else if (handle->state == ESTAB){
            handle->rwnd += htonl(header->ack_num)- handle->lastack;
            handle->lastack = (uint32_t)htonl(header->ack_num);
            handle->acknum = htonl(header->seq_num) + data_size - 20;
       ...
       //     flush_buf(tcp_context, handle);
            ...
            while(handle->buflen > 0)
            {
                ...
                header->checksum = tcp_checksum(tcp_data, handle->source->sin_addr, handle->dest->sin_addr,  20+real_data_size,real_data_size);
                memcpy(tcp_data,header,20);
                tcp_context->ktcp_lib->tcp_dispatch_ip(handle->source->sin_addr,handle->dest->sin_addr,tcp_data,20+ real_data_size);
                //free(tcp_data);
...
void send_ack(global_context_t* tcp_context, ctx* handle, tcp_header* header, void* data, int data_size){
    if (data_size > 20){
            ...
            header->checksum = tcp_checksum(header, handle->source->sin_addr, handle->dest->sin_addr,  20, 0);
            tcp_context->ktcp_lib->tcp_dispatch_ip(handle->source->sin_addr,handle->dest->sin_addr,header,20);
            void* tcp_data = malloc(data_size-20);
            memcpy(tcp_data,data+20,data_size-20);
            tcp_context->ktcp_lib->tcp_dispatch_app(handle,tcp_data,data_size-20);
            free(tcp_data); 
    }
}
...
void flush_buf(global_context_t* tcp_context, ctx* handle){
        while(handle->buflen > 0)
        {
            ...
            if(handle->rwnd - real_data_size < 0)
                break;
            void *tcp_data = malloc(20+real_data_size);
            tcp_header *header = make_header(htons(handle->source->sin_port),htons(handle->dest->sin_port),handle->seqnum, handle->acknum, (uint8_t)0x50, (uint8_t)0x10, 3072, 0, 0);
            memcpy(tcp_data,header,20);
            if(real_data_size + handle->start > BUFMAX)
            {
                memcpy(tcp_data+20,handle->buffer+handle->start,BUFMAX-handle->start);
